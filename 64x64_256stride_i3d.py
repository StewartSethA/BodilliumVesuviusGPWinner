...
def get_xyxys(fragment_ids, cfg, is_valid=False, start_idx=15, end_idx=45, train_images={}, train_masks={}, train_ids=[], pads={}, scale=1):
    ...
    for fragment_id in fragment_ids:
        myscale = scale
        if fragment_id in ['20231215151901', '20231111135340', '20231122192640']: #, '20231122192640']:
...
class RegressionPLModel(pl.LightningModule):
    def __init__(self,pred_shape=None,size=256,enc='',with_norm=False,total_steps=500, train_dataset=None, check_val_every_n_epoch=1, backbone=None, wandb_logger=None, val_masks=None,name="", complexity=16):
...
#        self.mask_count = np.zeros(self.hparams.pred_shape)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.writer.add_scalar("loss_mse/train", mseloss, self.current_epoch * len(self.training_dataloader) + batch_idx)
        self.writer.add_scalar("loss_mad/train", madloss, self.current_epoch * len(self.training_dataloader) + batch_idx)
        self.writer.add_scalar("loss_bce/train", bceloss, self.current_epoch * len(self.training_dataloader) + batch_idx)
        self.writer.add_scalar("loss_dice/train", diceloss, self.current_epoch * len(self.training_dataloader) + batch_idx)
        self.writer.add_scalar("loss/train", loss1, self.current_epoch * len(self.training_dataloader) + batch_idx)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        x,y,xyxys,ids= batch
        if [i == "20231012184423"].any() or self.current_epoch > 4:
          pass
        else:
          return #continue
        batch_size = x.size(0)
