...
CFG.scale = 8 #1024
#CFG.tile_size = 64
CFG.train_batch_size = CFG.valid_batch_size = 256
CFG.tile_size = 256 #1024 #8
CFG.size = 128 #8 # WAS 64
CFG.stride = 64 # WAS 64
CFG.valid_id = "20231210132040" if len(sys.argv) == 1 else sys.argv[1]
CFG.basepath = os.path.join(os.path.dirname(os.path.abspath(os.getcwd())), "Vesuvius")
#cfg_init(CFG)
...
for xoffset in range(scrollstart, 4096, 512): #CFG.size): #512, 1024: #4, 8: #32, 16, 8, 48, 56: # 512 is a typical upper bound on character size
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[StochasticWeightAveraging(5e-6, 3, 3), ModelCheckpoint(filename=f'wild12_64_Scroll1_{fid}_{fold}_fr_{enc}_scale{CFG.scale}'+f'xoffset{xoffset}',dirpath=CFG.model_dir,monitor='train/total_loss',mode='min',save_top_k=CFG.epochs),],
    )

    train_ids = ['20230702185753','20230929220926','20231005123336','20231012184423','20231007101619','20231016151002','20231022170901','20231031143852','20231106155351','20231210121321','20231221180251','20230820203112']
    ...
    train_ids = set([CFG.valid_id] + train_ids) #['20231012184423'])
    #train_ids = set(['20230702185753']) #,'20230929220926','20231005123336','20231012184423','20231007101619','20231016151002','20231022170901','20231031143852','20231106155351','20231210121321','20231221180251','20230820203112']) - set([CFG.valid_id])
    #train_ids = set(['20231210132040']) #, '20230702185753','20230929220926','20231005123336','20231012184423','20231007101619','20231016151002','20231022170901','20231031143852','20231106155351','20231210121321','20231221180251','20230820203112']) - set([CFG.valid_id])
    ...
    train_images[fid] = train_images[fid][vtrim:train_images[fid].shape[0]-vtrim*2,xoffset:totwidth+xoffset]
    ...
    train_masks[fid] = train_masks[fid][vtrim:train_masks[fid].shape[0]-vtrim*2,xoffset:totwidth+xoffset,:]
